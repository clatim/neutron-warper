...
def create_material_field(materials: MaterialConfiguration, geometry: fem.Geometry, property: str, material_ids: np.array) -> fem.Field:
    ...
    for id in np.nditer(material_ids):
        material_values.append(
            getattr(materials.materials[int(id)], property)
        )
